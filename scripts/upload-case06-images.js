...
initializeApp({
  ...
  storageBucket: 'ai-accounting-adedf.firebasestorage.app',
});
...
const images = [
  { file: 'case06_img1_dashboard.png', dest: 'cases/case06/img1_dashboard.png', mime: 'image/png' },
  { file: 'case06_img2_sheets.png',    dest: 'cases/case06/img2_sheets.png',    mime: 'image/png' },
  { file: 'case06_img3_trend.png',     dest: 'cases/case06/img3_trend.png',     mime: 'image/png' },
  { file: 'case06_img4_debug.png',     dest: 'cases/case06/img4_debug.png',     mime: 'image/png' },
];
...
async function uploadImage({ file, dest, mime }) {
  ...
  await fileRef.save(fileBuffer, { metadata: { contentType: mime }, public: true });
...
async function run() {
  ...
  for (const img of images) {
    urls[img.file] = await uploadImage(img);
  }

  // 이미지 URL이 포함된 최종 content로 Firestore 업데이트
...
![피드 시트 구조 — 7개 컬럼으로 모든 재무 데이터를 관리한다](${urls['case06_img2_sheets.png']})
...
![완성된 종합현황 대시보드 — KPI 카드 4개, PL 테이블, 분기 차트, BS 요약, 재무비율 카드](${urls['case06_img1_dashboard.png']})
...
**문제 3.** BS 데이터에서 대손충당금 파싱 오류도 발생했다.

대손충당금은 매출채권에서 차감하는 항목이라 음수값(-854,469,316)이어야 했는데, 시트 구조상 차감 후 소계값(67,239,770,583)이 들어온 것이었다. BS 시트의 컬럼 구조가 \`계정명(C) | 차감값(D) | 소계(E)\`로 되어 있었고, 스크립트가 E열(소계)만 읽고 있었던 것이다.

![Claude가 BS 파싱 오류의 원인을 찾아 수정 방법을 설명하는 장면](${urls['case06_img4_debug.png']})

Claude Code가 C열과 D열을 함께 읽도록 \`parseBS\` 함수를 수정해서 해결했다.
...
![손익추이 페이지 — 월별/분기별/연도별 탭 전환, 전기 대비 비교 차트와 상세 테이블](${urls['case06_img3_trend.png']})
...
CSS 503, 계정과목 매핑 오류, BS 파싱 오류, Edge Runtime 충돌 — 모두 Claude Code가 로그와 데이터를 직접 읽고 원인을 찾았다. 오류를 마주쳤을 때 당황하지 않고 Claude Code에 그대로 보여주면 된다.
...
*이전 글: [케이스 05 — SAP 자동화를 매일 자동 실행되게 만든 방법](/cases/case05-sap-scheduler-automation)*`;
  ...
  await db.collection('cases').doc('case06-management-dashboard-build').update({ content });
  console.log('case06 content updated');
  process.exit(0);
}
